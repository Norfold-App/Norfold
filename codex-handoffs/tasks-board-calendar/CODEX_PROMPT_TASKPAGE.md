...
> **⛓ RUN ORDER — see `../MASTER_ORDER.md` (step 5).** This is a **visual/layout pass ONLY — keep `AdaptiveTaskPage`'s signature unchanged.** `../tasks-notes-overhaul/01` (later) is the sanctioned place to add a create-mode param + new call sites — do NOT do that here. The standalone **Notes card** you build here (delta 5) later gets the live editor from `../tasks-notes-overhaul/03`; just style it now. Satisfy the universal Definition-of-Done GATE before declaring done.
